  ...
  rows.forEach(function (row) {
    ...
    for (field in row) {
      if (row.hasOwnProperty(field)) {
        ...
        if (field === "timestampBooking" || field === "timestampOptinMailSent" || field === "timestampConfirmationReceived") row[field] = new Date(parseInt(row[field] + '000'));

        if (field === 'participant_salutation') row[field] = (row[field].toLowerCase() !== 'herr') ? 'female' : 'male';
