  ...
  async.eachSeries(dbConv.config.mapping, function(mapping, seriesCb) {
      async.auto({
          ...
          collect: ['init', function(cb, config) {
            dbConv.log.verbose('Collection data from table `%s`...',
              config.init
              .tableFrom);

            hosts.fromHost.findAll(function(err, data) {
              dbConv.log.info(
          ...
          convert: ['collect', 'drop', function(cb, entries) {
            ...
            var converted = [];
            async.each(entries.collect, function(entry, callback) {
              converted.push(dbConv.converter(entry, mapping,
                callback));
            }, function(err) {
              dbConv.log.info(
