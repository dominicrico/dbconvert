...
module.exports = function(dbConvert) {
  ...
  return function loadConfig(cb) {
    ...
    async.auto({
            ...
            async.auto({
              fromHost: function(callback) {
                if (typeof overrides.mapOverrides.fromHost !==
                  ...
                  .fromHost.adapter) {
                  throw Error(
                    'No adapter specified for `fromHost`, but we need one!'
                  );
                } else {
                  callback();
                }
              ...
              toHost: ['fromHost',
                function(callback) {
                  if (typeof overrides.mapOverrides.toHost !==
                    ...
                    .toHost.adapter) {
                    throw Error(
                      'No adapter specified for `toHost`, but we need one!'
                    );
                  } else {
                    callback();
                  }
        ...
        logger: ['connections',
          function(cb, overrides) {
            dbConvert.log = winston;
            dbConvert.log.level = overrides.connections.log;
            dbConvert.log.add(winston.transports.File, {
              filename: overrides.connections.logPath +
                '/db-convert.log'
            });

            dbConvert.log.setLevels(winston.config.npm.levels);
            dbConvert.log.addColors({
              error: 'red',
              warn: 'yellow',
              info: 'blue',
              verbose: 'green',
        ...
        mappings: ['logger',
          function(cb, overrides) {
            require('./mappings')(cb, overrides);
          }
        ],

        relations: ['mappings',
          function(cb, overrides) {
            require('./relations')(cb, overrides);
          }
        ],
      },
  ...
  function guessAdapter(port) {
    ...
    dbConvert.log.silly('Using %s for now...', adapterByPort[parseInt(port)]);
